...
<script type="text/javascript">
	...
	function createCusForm(){
		...
		var cusBtn = document.querySelector(".createCustomerBtn");
			...
			if (CusName.value == "") {
				...
				var url = "{{ route('admin.createCustomer') }}";
	...
	function getCustomerData() {
		...
		var url = "{{ route('admin.GetCustomer') }}";
		axios.get(url)
		.then(function(response){
		if (response.status == 200) {
	...
	function customerShow(ShowId){
		var url = "{{ route('admin.CustomerShow') }}";
	...
	function customerUpdate(){
		...
		cusUpForm.addEventListener("submit",function(e){
			...
			var url = "{{ route('admin.customerUPdate') }}";
	...
	function customerDelete(deleteId){
		Swal.fire({
		  ...
		  if (result.isConfirmed) {
		   var url = "{{ route('admin.CustomerDelete') }}";
